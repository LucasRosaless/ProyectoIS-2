...
public class App {
    ...
    public static void main(String[] args) {
        ...
        // POST: Maneja el envío del formulario de creación de profesor.
        post("/profesor", (req, res) -> {
            ...
            // Validaciones básicas: todos los campos son requeridos.
            if (name == null || name.isEmpty() || email == null || email.isEmpty() ||
                department == null || department.isEmpty() || password == null || password.isEmpty()) {
                res.status(400); // Bad Request.
                res.redirect("/profesor?error=Todos los campos son requeridos.");
                return "";
            }

            try {
                Profesor newProfessor = new Profesor();
                String hashedPassword = BCrypt.hashpw(password, BCrypt.gensalt());

                newProfessor.set("name", name);
                newProfessor.set("email", email);
                newProfessor.set("department", department);
                newProfessor.set("password", hashedPassword);
                newProfessor.saveIt();

                res.status(201); // Código de estado HTTP 201 (Created) para una creación exitosa.
                // Redirige al dashboard con un mensaje de éxito.
                res.redirect("/dashboard?message=Profesor " + name + " registrado exitosamente!");
                return ""; // Retorna una cadena vacía.

            } catch (Exception e) {
                // Si ocurre cualquier error durante la operación de DB (ej. email duplicado),
                // se captura aquí y se redirige con un mensaje de error.
                System.err.println("Error al registrar el profesor: " + e.getMessage());
                e.printStackTrace(); // Imprime el stack trace para depuración.
                res.status(500);
                res.redirect("/profesor?error=Error interno al registrar profesor. Asegúrese que el correo no esté duplicado.");
                return ""; // Retorna una cadena vacía.
            }
        });
